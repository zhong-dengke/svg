<template>
  <div class="topbar">
    ...
    <img
      class="config-logo-style map-logo-style"
      src="../../assets/images/map.png"
    />
    <span
      class="parameter-configuration digital-map"
      @click="$router.push({ name: 'digitalMap' })"
      >数字地图</span
    >
    <img
      class="config-logo-style"
      src="../../assets/images/parameterConfiguration.png"
    />
    <span
    ...
    <span class="user-name" @click="loginOut">admin</span>
  </div>
</template>
...
export default {
  ...
  watch: {
    $route(val) {
      if (val.name === "index" || val.name === "login") {
        this.$store.commit("setShowState", false);
      } else this.$store.commit("setShowState", true);
    }
  },
  mounted() {
    this.tunnels = [
